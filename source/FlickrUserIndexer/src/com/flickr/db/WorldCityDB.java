...
public class WorldCityDB {
	...
	public boolean insertPlace(Place place){
		String[] names = { "place_id","place_longitude","place_latitude", "country_code"};
		String[] values = { place.getPlaceId(), place.getPlaceLong(),place.getPlaceLat(), place.getCountryCode()};
		if (DBManager.insert("place", names, values) != 0) {
			return true;
		} else {
			return false;
		}
...
//	private boolean checkPlace(String placeId){
//		String query = "SELECT place_id FROM user WHERE place_id='"+placeId+"'";
//		ResultSet rs = DBManager.ExecuteQuery(query);
//		try {
//			return rs.first();
//		} catch (SQLException e) {
...
//	}
	...
	public boolean IsInsertedPlace(String placeId){
		String query = "SELECT * FROM place WHERE place_id='"+placeId+"'";
		ResultSet rs = DBManager.ExecuteQuery(query);
		try {
			return rs.first();
		} catch (SQLException e) {
